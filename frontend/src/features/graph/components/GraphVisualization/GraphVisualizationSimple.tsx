...
export const GraphVisualizationSimple = () => {
  ...
  const [currentLayout, setCurrentLayout] = useState<
    'circle' | 'cose' | 'distance'
  >('cose');
  const [centerNodeId, setCenterNodeId] = useState<string | null>(null);
  const [nodeDistances, setNodeDistances] = useState<Map<string, number>>(
    new Map()
  );
  ...
  const calculateEdgeWeight = (edge: any) => {
    ...
    if (
      fact.toLowerCase().includes('member of') ||
              ...
              if (currentLayout === 'distance') {
                ...
                const layoutOptions = {
                  ...
                  concentric: function (n: any) {
                    const distance =
                      nodeDistances.get(n.data('id')) || Infinity;
    ...
    if (cyRef.current) {
      ...
      if (newLayout === 'distance' && nodeDistances.size > 0) {
        ...
        layoutOptions = {
          ...
          levelWidth: function (_nodes: any) {
